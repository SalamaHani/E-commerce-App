...
import { removProductF } from "../features/Cart/cartSlice";

import { useSelector, useDispatch } from "react-redux";
import { formatPrice } from "../Util";

function ProductcartFava({ item }) {
  const { cartID, image, name, price } = item;
  const carts = useSelector((state) => state.cartState.Favorites);
  const Favorites = carts.some((item) => item.cartID == cartID);
  const dicpatch = useDispatch();
  const handeltogel = () => {
    dicpatch(removProductF(cartID));
  };
  return (
    <article className="relative">
      <Link to={`/Products/${cartID}`} className="shadow-2xl ">
...
export default ProductcartFava;
